...
//Module responsible for outputting to SSEG
module rotating_number_banner
#(parameter PRESCALER_WIDTH = 16,                        //Controls the width of the timer 
            SSEG_COUNT_WIDTH = 2)                         //Controls width of sseg switch (an)                       
(
   input logic clk, reset,
   input logic dir, enable,                                      //Constrol direction of rotation
...
shift_reg_4bit#(.PRESCALER_WIDTH(26)) shift_reg(.clk(clk), .reset(reset), .dir(dir), .enable(enable), .parallel_out(hex_parallel_out));
